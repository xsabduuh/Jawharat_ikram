...
function addToCart(id) {
  ...
  saveCart();
  updateCartBadge();
  showToast(`تمت إضافة "${product.name}" للسلة`, 'fas fa-check-circle');
}
...
// عرض محتوى السلة (يتم استدعاؤها من الصفحة الرئيسية)
function renderCart() {
  ...
  if (cart.length === 0) {
    wrap.innerHTML = `
      <div class="empty-cart">
        ...
        <button class="btn-primary" onclick="navigate('products')"><i class="fas fa-shopping-bag"></i> تسوقي الآن</button>
      </div>`;
    return;
  }
...
// يمكن استدعاء هذه الدالة من الخارج أيضاً (إذا أردتِ توجيه إشعار)
